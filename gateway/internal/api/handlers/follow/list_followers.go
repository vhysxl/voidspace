...
func (h *FollowHandler) ListFollowers(c echo.Context) error {
	...
	followers, err := h.UserService.ListFollowers(ctx, int64(targetUser.ID))
	if err != nil {
		return utils.HandleDialError(h.Logger, c, err, "failed to list followers")
	}

	return responses.SuccessResponseMessage(c, http.StatusOK, constants.ListFollowersSuccess, followers)
}
