...
func (ps *PostService) GetGlobalFeed(ctx context.Context, req *postpb.GetGlobalFeedRequest, reqUserID string, reqUsername string) (*models.GetFeedResponse, error) {
	...
	res, err := ps.PostClient.GetGlobalFeed(ctx, req)
	if err != nil {
		ps.Logger.Error("failed to call PostService.GetGlobalFeed", zap.Error(err))
		return nil, err
	}
