...
func (p *profileRepository) GetProfileById(ctx context.Context, userId int32) (*domain.Profile, error) {
...
func (p *profileRepository) Update(ctx context.Context, userId int32, profile *domain.Profile) error {
	setClauses := []string{
		"display_name = ?",
		"bio = ?",
		"avatar_url = ?",
	...
	query := fmt.Sprintf(
		"UPDATE user_profile SET %s WHERE user_id = ?",
